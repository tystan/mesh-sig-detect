...
While not the values used for determining significance, they are the statistics that are most familiar to interested parties.
The RR estimates for maxSPRT and PRR methods are of course identical, the methods differ in using the LLR and Wald testing respectively.
The IC statistic is not directly comparable to RR, it does share similarities in that values above the null value represent disproportionate reporting rate in the cohort of interest.
(include a 2^IC vs RR numeric example here)
...
The Henernia mesh v Other mesh group serves a negative control as it is not expected to produce a significant signal. Indeed no signal detection method reaches significance for this comparison.
