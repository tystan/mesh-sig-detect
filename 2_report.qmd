...
The aim of this study was to develop an in-principle, end-to-end pipeline to complement traditional signal detection of disproportionately higher reporting rates of adverse events, using a historical true-positive pelvic mesh, to utilise information currently not used to screen for medical device safety issues to:

1. classify the DAEN reports into topics, based on the free-text descriptions of the adverse events, using natural language processing (NLP) 
2. implement best-in-class, repeated looks over time signal detection algorithms on NLP process data 
3. report time to signal findings for the true-positive pelvic mesh safety for generalisation and feasibility
3. compare the screening detection timings to more widespread concerns
